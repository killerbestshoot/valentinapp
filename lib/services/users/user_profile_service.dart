...
import 'package:mon_premye_app/services/auth/local_profile_store.dart';
...
class UserProfile {
  ...
  factory UserProfile.fromMap({
    ...
    return UserProfile(
      authUid: authUid,
      userId: AppIds.isPrefixedUuid5(userId)
          ? userId
          : AppIds.userForRole(_text(data['role']), seed: authUid),
...
class UserProfileService {
  ...
  Map<String, dynamic> _buildProfilePayload({
    ...
    final userId = AppIds.isPrefixedUuid5(existingUserId)
        ? existingUserId
        : AppIds.userForRole(
            resolvedRole.isEmpty ? defaultRole : resolvedRole,
            seed: user.uid,
          );
