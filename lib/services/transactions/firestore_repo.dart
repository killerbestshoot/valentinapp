...
class FirestoreRepo {
  ...
  Future<void> createTransaction({
    ...
    final staffName = (staff['displayName'] ?? staff['fullName'] ?? 'Agent')
        .toString()
        .trim();
    ...
    final existingTxId =
        (payload['txId'] ?? payload['transactionId'] ?? '').toString().trim();
    final txId = AppIds.isPrefixedUuid5(existingTxId)
        ? existingTxId
        : AppIds.transaction(
            seed: '$enterpriseId:$staffUid:${now.toIso8601String()}');
    ...
    final paymentAmount =
        asDouble(payload['paymentAmount'] ?? payload['amount']);
    final transferAmount =
        asDouble(payload['transferAmount'] ?? payload['amount']);
    ...
    await _db.collection('transactions').doc(txId).set(data);
  }
}
