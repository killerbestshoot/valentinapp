...
class WalletEngine {
  ...
  static Future<void> creditBalance({
    ...
    final ledgerId = AppIds.ledger(
      seed: 'credit:$enterpriseId:$uid:$type:${DateTime.now()}',
    );
    final historyId = AppIds.history(
      seed: 'credit:$enterpriseId:$uid:$type:${DateTime.now()}',
    );
    final ledgerRef = _db.collection('ledger').doc(ledgerId);
    final historyRef = _db.collection('wallet_history').doc(historyId);
  ...
  static Future<void> debitBalance({
    ...
    final balanceRef = _db.collection('balances').doc('${enterpriseId}_$uid');
    final ledgerId = AppIds.ledger(
      seed: 'debit:$enterpriseId:$uid:$type:${DateTime.now()}',
    );
    final historyId = AppIds.history(
      seed: 'debit:$enterpriseId:$uid:$type:${DateTime.now()}',
    );
