...
class WalletService {
  ...
  static Future<void> topupWallet({
    ...
    final logId = AppIds.walletLog(seed: 'topup:$targetUid:${DateTime.now()}');
    final logRef = _db.collection('wallet_logs').doc(logId);
  ...
  static Future<void> lockBalance({
    ...
    await _db.runTransaction((tx) async {
      ...
      final logId = AppIds.walletLog(seed: 'lock:$uid:${DateTime.now()}');
      final log = _db.collection('wallet_logs').doc(logId);
  ...
  static Future<void> submitWithdrawRequest({
    ...
    final requestId = AppIds.withdrawRequest(
      seed: '${user.uid}:$enterpriseId:${DateTime.now().toIso8601String()}',
    );

    await _db.collection('payout_requests').doc(requestId).set({
      'requestId': requestId,
      'payoutId': requestId,
      'uid': user.uid,
      'email': user.email ?? '',
  ...
  static Future<void> approveWithdraw({
    ...
    await _db.runTransaction((tx) async {
      ...
      final logId = AppIds.walletLog(
        seed: 'withdraw_approved:$uid:${DateTime.now()}',
      );
      final log = _db.collection('wallet_logs').doc(logId);
  ...
  static Future<void> rejectWithdraw({
    ...
    await _db.runTransaction((tx) async {
      ...
      final logId = AppIds.walletLog(
        seed: 'withdraw_rejected:$uid:${DateTime.now()}',
      );
      final log = _db.collection('wallet_logs').doc(logId);
