...
class WalletTopupRequestService {
  ...
  static Future<Map<String, dynamic>> findStaffInEnterprise(
      String query) async {
  ...
  static Future<void> createTopupRequest({
    ...
    final requestId = AppIds.topupRequest(
      seed:
          '${profile['enterpriseId']}:${targetStaff['uid']}:${DateTime.now().toIso8601String()}',
    );

    await _db.collection('wallet_topup_requests').doc(requestId).set({
      'requestId': requestId,
      'topupId': requestId,
      'type': 'wallet_topup',
      'status': 'pending',
  ...
  static Future<void> approveTopupRequest(String requestId) async {
    ...
    await _db.runTransaction((tx) async {
      ...
      final ledgerId = AppIds.ledger(seed: 'topup:$requestId:$targetUid');
      final ledgerRef = _db.collection('wallet_ledger').doc(ledgerId);
      tx.set(ledgerRef, {
        'ledgerId': ledgerId,
        'type': 'wallet_topup',
  ...
  static Future<void> rejectTopupRequest(String requestId,
      {String note = ''}) async {
    final profile = await getMyProfile();
    final role = (profile['role'] ?? '').toString();
