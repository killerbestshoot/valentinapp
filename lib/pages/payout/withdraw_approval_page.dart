...
import 'package:mon_premye_app/services/wallet/wallet_service.dart';
...
class WithdrawApprovalPage extends StatelessWidget {
  ...
  Future<void> approve(BuildContext context, QueryDocumentSnapshot doc) async {
    try {
      ...
      final logId = AppIds.payoutLog(seed: 'withdraw:${doc.id}');
      final logRef = db.collection('payout_logs').doc(logId);
      ...
      await db.runTransaction((tx) async {
        ...
        if (!walletSnap.exists) {
          throw Exception('Wallet pa egziste pou user sa a');
        }
        ...
        if (enterpriseBalance < amount) {
          throw Exception(
              'Enterprise balance pa sifi ($enterpriseBalance < $amount)');
        }

        if (walletReserved < amount) {
          throw Exception(
              'Reserved pa sifi nan wallet ($walletReserved < $amount)');
        }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Request approved + reserved retire nan wallet')),
      );
    } catch (e) {
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Expanded(
                        child: Center(
                          child: Text(
                              'Pa gen request pending pou enterprise sa a'),
                        ),
                      ),
                    ],
                  ),
                ...
                children: [
                  ...
                  ...docs.map((doc) {
                    ...
                    return Card(
                      child: ListTile(
                        title: Text(
                            '${amount.toStringAsFixed(2)} ${data['currency'] ?? 'USD'}'),
                        subtitle: Text(
