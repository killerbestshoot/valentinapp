                          ...
                          return StreamBuilder<
                            ...
                            builder: (context, txSnap) {
                              ...
                              if (payoutSnap.hasError) {
                                return Center(
                                    child: Text(
                                        'Erreur payouts: ${payoutSnap.error}'));
                              }
                              if (serviceSnap.hasError) {
                                return Center(
                                    child: Text(
                                        'Erreur services: ${serviceSnap.error}'));
                              }
                              if (userSnap.hasError) {
                                return Center(
                                    child: Text(
                                        'Erreur users: ${userSnap.error}'));
                              }
                              if (txSnap.hasError) {
                                return Center(
                                    child: Text(
                                        'Erreur transactions: ${txSnap.error}'));
                              }
                              ...
                              for (final d in payoutDocs) {
                                ...
                                alerts.add({
                                  'level':
                                      amount >= 1000 ? 'critical' : 'warning',
                                  'title': 'Pending payout request',
                                  'subtitle':
                                      'Service: $serviceName | UID: $uid',
                              ...
                              for (final d in inactiveServiceDocs) {
                                ...
                                alerts.add({
                                  ...
                                  'subtitle':
                                      '$name | $category | Enterprise: $enterpriseId',
                                  'value': 'inactive',
                              ...
                              for (final d in inactiveUserDocs) {
                                ...
                                alerts.add({
                                  ...
                                  'title': 'Inactive user',
                                  'subtitle':
                                      '$name | $role | Enterprise: $enterpriseId',
                                  'value': d.id,
                                  'time': _fmtTs(m['updatedAt']),
                              ...
                              for (final d in txDocs) {
                                ...
                                final status = (m['status'] ?? '')
                                    .toString()
                                    .toLowerCase();
                                final paymentStatus = (m['paymentStatus'] ?? '')
                                    .toString()
                                    .toLowerCase();
                                ...
                                final commissionApplied =
                                    m['commissionApplied'] == true;
                                if (status == 'delivered' &&
                                    paymentStatus == 'paid' &&
                                    !commissionApplied &&
                                ...
                                return ListView(
                                  children: [
                                    Wrap(
                                      ...
                                      runSpacing: 12,
                                      children: [
                                        _statBox('Critical', '$criticalCount',
                                            Colors.red),
                                        _statBox('Warning', '$warningCount',
                                            Colors.orange),
                                        _statBox(
                                            'Info', '$infoCount', Colors.blue),
                                      ],
                                    ),
