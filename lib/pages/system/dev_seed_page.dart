...
class _DevSeedPageState extends State<DevSeedPage> {
  ...
  Future<void> seedDemoData() async {
    try {
      ...
      final pendingId = AppIds.payoutRequest(seed: 'seed:pending:$uid');
      final approvedId = AppIds.payoutRequest(seed: 'seed:approved:$uid');
      final rejectedId = AppIds.payoutRequest(seed: 'seed:rejected:$uid');

      await payoutRequests.doc(pendingId).set({
        'requestId': pendingId,
        'payoutId': pendingId,
        'action': 'pay',
        'uid': uid,
      ...
      await payoutRequests.doc(approvedId).set({
      ...
      await payoutRequests.doc(rejectedId).set({
      ...
      final payoutLogId = AppIds.payoutLog(seed: 'seed:$pendingId');
      await payoutLogs.doc(payoutLogId).set({
        'logId': payoutLogId,
        'payoutRequestId': pendingId,
        'uid': uid,
        'enterpriseId': enterpriseId,
        'serviceId': 'moncash_ht',
        'serviceName': 'MonCash',
      ...
      final submittedNotificationId = AppIds.notification(
        seed: 'seed:payout_submitted:$pendingId',
      );
      await notifications.doc(submittedNotificationId).set({
      ...
      final approvedNotificationId = AppIds.notification(
        seed: 'seed:payout_approved:$approvedId',
      );
      await notifications.doc(approvedNotificationId).set({
        'notificationId': approvedNotificationId,
      ...
      final rejectedNotificationId = AppIds.notification(
        seed: 'seed:payout_rejected:$rejectedId',
      );
      await notifications.doc(rejectedNotificationId).set({
        'notificationId': rejectedNotificationId,
