...
class _ReceiptValidationPageState extends State<ReceiptValidationPage> {
  ...
  String _buildSignature({
    ...
    final raw =
        '$txId|$enterpriseId|$paymentAmount|$paymentCurrency|$serviceId|$_receiptSecret';
  ...
  Future<void> _writeValidationLog({
    ...
    final db = FirebaseFirestore.instance;
    final validationId = AppIds.validationLog(
      seed: '${_s(receiptData['txId'])}:$status:$validatedBy',
    );

    await db.collection('receipt_validation_logs').doc(validationId).set({
