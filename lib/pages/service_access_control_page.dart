...
class ServiceAccessControlPage extends StatefulWidget {
  ...
  @override
  State<ServiceAccessControlPage> createState() => _ServiceAccessControlPageState();
}
...
class _ServiceAccessControlPageState extends State<ServiceAccessControlPage> {
  ...
  Future<Map<String, dynamic>> _loadEnterpriseContext() async {
    ...
    final enterpriseName = (userData['enterpriseName'] ?? 'VOUPVAPCASH').toString();
  ...
  Future<void> _toggleAccess({
    ...
    try {
      ...
      final ref = FirebaseFirestore.instance.collection('service_access').doc(accessId);
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (manageableUsers.isEmpty)
                              const Text('Pa gen user aktif pou jere aks yo kounye a.')
                            else
                              DropdownButtonFormField<String>(
                                ...
                                items: manageableUsers.map((doc) {
                                  ...
                                  return DropdownMenuItem<String>(
                                    value: doc.id,
                                    child: Text('$displayName - $role - $email'),
                                  );
                                }).toList(),
                                onChanged: (value) {
                                  setState(() {
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (serviceDocs.isEmpty)
                                    const Text('Pa gen Sevis aktif pou asiyen kounye a.')
                                  else
                                    ...serviceDocs.map((serviceDoc) {
                                      final serviceId = serviceDoc.id;
                                      final serviceData = serviceDoc.data();
                                      final serviceName = _text(serviceData['name'], serviceId);
                                      final category = _text(serviceData['category']);
                                      final isAllowed = accessIds.contains(serviceId);
