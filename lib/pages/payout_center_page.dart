...
class _PayoutCenterPageState extends State<PayoutCenterPage> {
  ...
  Future<Map<String, dynamic>> _loadCurrentUser() async {
    ...
    return {
      'uid': uid,
      'displayName': (data['displayName'] ?? data['fullName'] ?? 'Staff').toString(),
  ...
  Future<void> _approvePayout({
    ...
    try {
      ...
      final payoutRef = FirebaseFirestore.instance.collection('payout_requests').doc(docId);
      ...
      await FirebaseFirestore.instance.runTransaction((tx) async {
        ...
        final logRef = FirebaseFirestore.instance.collection('payout_logs').doc();
        tx.set(logRef, {
  ...
  Future<void> _rejectPayout({
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('payout_requests').doc(docId).set({
        'status': 'rejected',
        'processed': true,
        'paidOut': false,
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: Text(_saving ? 'Saving...' : 'Create Payout Request'),
                          ),
                        ),
                      ],
                    ),
                ...
                Expanded(
                  child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                    ...
                    builder: (context, snap) {
                      ...
                      if (snap.hasError) {
                        return Center(child: Text('Erreur payout requests: ${snap.error}'));
                      }
                        ...
                        children: [
                          Wrap(
                            ...
                            children: [
                              ...
                              _statBox('Pending Total', '${pendingTotal.toStringAsFixed(2)} USD', Colors.blue),
                              _statBox('Approved Total', '${approvedTotal.toStringAsFixed(2)} USD', Colors.purple),
                            ],
                          ),
                          ...
                          if (docs.isEmpty)
                            ...
                            ...docs.map((d) {
                              ...
                              final serviceId = (m['serviceId'] ?? '').toString();
                              ...
                              return Card(
                                child: ExpansionTile(
                                  ...
                                  title: Text('${amount.toStringAsFixed(2)} USD'),
                                  subtitle: Text(
                                    'Status: $status\n'
