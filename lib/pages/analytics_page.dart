            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, snap) {
                  ...
                  if (snap.hasError) {
                    return Center(child: Text('Erreur analytics: ${snap.error}'));
                  }

                  final docs = (snap.data?.docs ?? []).where((d) {
                    final m = d.data();
                    final txId = (m['txId'] ?? d.id).toString().toLowerCase();
                    final service = (m['serviceName'] ?? '').toString().toLowerCase();
                    final agent = (m['staffName'] ?? '').toString().toLowerCase();
                    final enterprise = (m['enterpriseId'] ?? '').toString().toLowerCase();
                  ...
                  for (final d in docs) {
                    ...
                    final agent = (m['staffName'] ?? 'Unknown Agent').toString();
                    ...
                    children: [
                      ...
                      if (docs.isEmpty)
                        const Card(
                          child: Padding(
                            padding: EdgeInsets.all(16),
                            child: Text('No transaction found for this filter.'),
                          ),
                        )
                      else
                        ...docs.take(20).map((d) {
                          ...
                          return Card(
                            child: ListTile(
                              ...
                              trailing: Text('${amount.toStringAsFixed(2)} USD'),
                            ),
                          );
                        }),
                    ],
