...
class _WalletTopupPageState extends State<WalletTopupPage> {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> _staffStream(String enterpriseId) {
  ...
  Future<void> _submitTopup({
    ...
    try {
      await FirebaseFirestore.instance.collection('wallet_topup_requests').add({
      ...
      builder: (context, profileSnap) {
        ...
        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, staffSnap) {
            ...
            final docs = staffSnap.data!.docs
                .map((e) => e.data())
                .where((e) {
                      ...
                      if (docs.isEmpty)
                        const Card(
                          child: Padding(
                            ...
                            child: Text('Pa gen staff ki koresponn ak rechch la'),
                          ),
                        ),
                      ...docs.map((staff) {
                        return Card(
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('Email: ${(staff['email'] ?? '').toString()}'),
                                Text('UID: ${(staff['uid'] ?? '').toString()}'),
                                Text('Role: ${(staff['role'] ?? '').toString()}'),
                                const SizedBox(height: 12),
                                SizedBox(
                                  ...
                                  child: ElevatedButton(
                                    onPressed: _loading
                                        ? null
                                        : () => _submitTopup(owner: owner, staff: staff),
                                    child: const Padding(
